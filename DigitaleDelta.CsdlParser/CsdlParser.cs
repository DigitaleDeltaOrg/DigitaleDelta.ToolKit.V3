...
public static class CsdlParser
{
    ...
    public static bool TryParse(string csdl, out CsdlModel? model, out string? error)
    {
        ...
        try
        {
            ...
            // Second pass: resolve ComplexType references
            ResolveComplexTypeReferences(model);
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Resolves ComplexType references in EntityType and ComplexType properties after initial parsing.
    /// Updates EdmType to EdmComplexType for properties that reference a defined ComplexType.
    /// </summary>
    /// <param name="model"></param>
    private static void ResolveComplexTypeReferences(CsdlModel model)
    {
        // Build a set of all defined ComplexType names (with and without namespace)
        var complexTypeNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

        foreach (var complexType in model.ComplexTypes)
        {
            complexTypeNames.Add(complexType.Name);
        }

        // Update EntityType properties
        foreach (var entityType in model.EntityTypes)
        {
            foreach (var property in entityType.Properties)
            {
                if (property.EdmType == EdmType.EdmUnknown && IsComplexTypeReference(property.Type, complexTypeNames))
                {
                    property.EdmType = EdmType.EdmComplexType;
                }
            }
        }

        // Update ComplexType properties (for nested ComplexTypes)
        foreach (var complexType in model.ComplexTypes)
        {
    ...
    /// <summary>
    /// Checks if a type string references a ComplexType.
    /// Handles both simple names and namespaced names (e.g., "Parameter" or "DigitaleDeltaModels.Parameter").
    /// </summary>
    /// <param name="typeString"></param>
    /// <param name="complexTypeNames"></param>
    /// <returns></returns>
    private static bool IsComplexTypeReference(string typeString, HashSet<string> complexTypeNames)
    {
        ...
        if (typeString.StartsWith("Collection(", StringComparison.OrdinalIgnoreCase) && typeString.EndsWith(")"))
        {
            cleanType = typeString.Substring(11, typeString.Length - 12);
        }

        // Check if it's an Edm type
        if (cleanType.StartsWith("Edm.", StringComparison.OrdinalIgnoreCase))
        {
            return false;
        }
