...
public static class ODataPropertyMapper
{
    ...
    private static Func<object, Dictionary<string, object?>> CreateMapperInternal(Type type, bool excludeNulls)
    {
        ...
        foreach (var property in type.GetProperties())
        {
            ...
            if (excludeNulls && (property.PropertyType.IsClass || Nullable.GetUnderlyingType(property.PropertyType) != null))
            {
                // compare boxed value to null so it works for Nullable<T> too
                var propIsNull = Expression.Equal(Expression.Convert(propAccess, typeof(object)), Expression.Constant(null, typeof(object)));
