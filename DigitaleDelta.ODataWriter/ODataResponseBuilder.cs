...
public class ODataResponseBuilder
{
    ...
    /// <summary>
    /// Configures pagination parameters for the OData response.
    /// </summary>
    /// <param name="skipToken">The skip token used to identify the starting point for pagination.</param>
    /// <param name="totalCount">The total count of records available. This parameter is optional.</param>
    /// <returns>An instance of <see cref="ODataResponseBuilder"/> for chaining further configurations.</returns>
    public ODataResponseBuilder WithPagination(string? skipToken, int? totalCount = null)
    {
        _skipTokenToUse = skipToken;
        _total = totalCount;
    ...
    public ODataResponseBuilder WithSelectProperties(IEnumerable<string?>? propertyNames)
    {
        ...
        _selectedProperties = propertyNames
    ...
    /// </summary>
    /// <returns></returns>
    public ODataResponse Build(bool applySort = false)
    {
        ...
        var selectedEntities = _entitiesToWrite.Select(entity => _selectedProperties.Count == 0
            ? entity
            : ApplySelect(entity, _selectedProperties, applySort)).ToList();
    ...
    private string SetNextLink()
    {
        ...
        parts.Add($"$skiptoken={System.Net.WebUtility.UrlEncode(_skipTokenToUse)}");
    ...
    private static Dictionary<string, object?> ApplySelect(IDictionary<string, object?> entity, IEnumerable<string> selectProperties, bool applySort)
    {
        ...
        return CopySelectedLevel(entity, wanted, prefix: null, sort: applySort);
    }
    ...
    /// <param name="sort">Sort the keys, if requested, but keep 'Id' first.</param>
    /// <returns>A new dictionary containing only the selected levels and keys.</returns>
    private static Dictionary<string, object?> CopySelectedLevel(IDictionary<string, object?> current, HashSet<string> wantedPaths, string? prefix, bool sort)
    {
        var result = new Dictionary<string, object?>(StringComparer.OrdinalIgnoreCase);
        var keys = sort
            ? current.Keys
                .OrderBy(k => k.Equals("Id", StringComparison.OrdinalIgnoreCase) ? 0 : 1)
        ...
        foreach (var key in keys)
        {
            ...
            if (value is IDictionary<string, object?> childDict)
            {
                if (parentSelected)
                {
                    result[key] = CopySelectedLevel(childDict, wantedPaths, fullKey, sort);
                    continue;
                }

                if (hasNestedSelections)
                {
                    var child = CopySelectedLevel(childDict, wantedPaths, fullKey, sort);
                    if (child.Count > 0)
                    {
                        result[key] = child;
                    }
